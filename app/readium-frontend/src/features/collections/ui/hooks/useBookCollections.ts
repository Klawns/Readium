...
export const useBookCollections = (bookId: number | null) => {
  ...
  const bookCollectionsQuery = useQuery({
    ...
    queryFn: () => useCases.listBookCollectionsUseCase.execute(bookId as number),
    enabled,
  });

  const setBookCollectionsMutation = useMutation({
    mutationFn: (collectionIds: number[]) => useCases.setBookCollectionsUseCase.execute(bookId as number, collectionIds),
