...
export const useBookCategories = (bookId: number | null) => {
  ...
  const bookCategoriesQuery = useQuery({
    ...
    queryFn: () => useCases.listBookCategoriesUseCase.execute(bookId as number),
    enabled,
  });

  const setBookCategoriesMutation = useMutation({
    mutationFn: (categoryIds: number[]) => useCases.setBookCategoriesUseCase.execute(bookId as number, categoryIds),
